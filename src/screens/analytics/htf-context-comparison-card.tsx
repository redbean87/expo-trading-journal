...
type HtfContextComparisonCardProps = {
  contexts: HtfContextMetrics[];
};

const COLUMNS = [
  { label: 'Context', flex: 2, align: 'left' as const },
...
export function HtfContextComparisonCard({
  contexts,
}: HtfContextComparisonCardProps) {
  const theme = useAppTheme();
  const styles = createStyles(theme);

  return (
    <SectionCard title="Performance by HTF Context">
      {contexts.length === 0 ? (
        <CardEmptyState
          icon="weather-windy"
          title="No HTF context data yet"
