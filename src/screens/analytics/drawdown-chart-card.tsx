      ...
      {!hasData ? (
        ...
        <>
          <View style={styles.chartRow}>
            <YAxis
              data={chartData}
              yAccessor={({ item }) => (item as ChartDataItem).drawdown}
              numberOfTicks={4}
              contentInset={yAxisInset}
              style={styles.yAxis}
              formatLabel={(value: number) => `$${Math.abs(value).toFixed(0)}`}
              svg={{
                fontSize: 10,
              ...
              {point && (
                <View style={styles.tooltipWrapper} pointerEvents="none">
                  <DataPointTooltip
                    ...
                    value={`-${formatCurrency(point.drawdown)}`}
