...
export function calculateTradeAnalytics(trades: Trade[]): TradeAnalytics {
  ...
  const longWinRate =
    longTrades.length > 0
      ...
      : 0;
  const longRR =
    longAvgLoss > 0 ? longAvgWin / longAvgLoss : longAvgWin > 0 ? Infinity : 0;
  ...
  // Overall R:R metrics
  const realizedRR = avgLoss > 0 ? avgWin / avgLoss : avgWin > 0 ? Infinity : 0;
  const lossRate =
    totalTrades > 0 ? (losingTrades.length / totalTrades) * 100 : 0;
  const expectedValue =
    totalTrades > 0 ? (winRate / 100) * avgWin - (lossRate / 100) * avgLoss : 0;
...
export function useTradeAnalytics(trades: Trade[]): TradeAnalytics {
  return useMemo(() => calculateTradeAnalytics(trades), [trades]);
}
