...
exports.isTosAccountStatement = isTosAccountStatement;
exports.parseTosAccountStatement = parseTosAccountStatement;
const papaparse_1 = __importDefault(require('papaparse'));
const trade_1 = require('../schemas/trade');
function isTosAccountStatement(content) {
...
function findSubset(fills, targetQty) {
...
function parseTosDateTime(dateStr, timeStr) {
...
function parseCashBalanceFills(section, errors) {
  const fills = [];
  papaparse_1.default.parse(section, {
    header: true,
    skipEmptyLines: true,
    complete: (results) => {
...
function parseTradeHistoryFills(section, errors) {
  const fills = [];
  papaparse_1.default.parse(section, {
    header: true,
    skipEmptyLines: true,
...
function emitTrade(symbol, accum, source) {
...
function parseTosAccountStatement(content) {
  const imported = [];
  const errors = [];
  let skipped = 0;
  ...
  const thSource = anyCbMatched ? 'tos-merged' : 'trade-history';
